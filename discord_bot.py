...
def setup_key_channel(guild_id, user_id):
    """Setup a key database channel - simple version"""
    try:
        # Create or use existing channel named "api-keys"
        channel_name = "api-keys"
        
        channel_data = {
            "name": channel_name,
            "type": 0,
            "topic": "API Keys Database - Private",
            "permission_overwrites": [
                {
                    "id": guild_id,
                    "type": 0,
        ...
        discord_api_request(f"/channels/{channel['id']}/messages", "POST", {
            "embeds": [embed],
            "content": "# API Keys Database"
        })
        
        logger.info(f"Created key channel: {channel['id']}")
        return channel['id']
        
    except Exception as e:
        logger.error(f"Error creating key channel: {e}")
        return None

# =============================================================================
# DISCORD BOT FUNCTIONS - FIXED
# =============================================================================

def test_discord_token():
    """Test if Discord token is valid - FIXED VERSION"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            bot_active = True
            logger.info(f"Discord bot connected: {bot_info['username']}")
            return True
        else:
            logger.error(f"Invalid Discord token: {response.status_code} - {response.text}")
            bot_active = False
            return False
...
def register_commands():
    ...
    commands = [
        ...
        {
            "name": "setup-keys",
            "description": "Setup API key channel (Admin only)",
            "type": 1
        }
    ]
        ...
        return {
            ...
            "data": {
                "content": (
                    ...
                    f"**Your API Key:**\n```{api_key}```\n\n"
                    f"Use `/key` to see your key again"
                ),
                "flags": 64
            }
        }
    ...
    return {
        ...
        "data": {
            "content": (
                ...
                f"**Name:** {in_game_name}\n"
                f"**API Key:**\n```{api_key}```\n\n"
                f"Use this key to login to the dashboard.\n"
...
def handle_close_command(data, user_id, user_name, server_id):
    ...
    if success:
        ...
        return {
            "type": 4,
            "data": {
                "content": f"Ticket marked as closed.",
                "flags": 64
            }
        }
...
def handle_setup_keys_command(data, user_id, user_name, server_id):
    ...
    is_admin = is_user_admin_in_guild(server_id, user_id)
    if not is_admin:
        return {"type": 4, "data": {"content": "You need admin privileges to setup key channel.", "flags": 64}}
    
    channel_id = setup_key_channel(server_id, user_id)
    
    if channel_id:
        return {
            "type": 4,
            "data": {
                "content": f"**Key Channel Created**\n\n**Channel:** <#{channel_id}>\n\nThis channel will store API keys.",
                "flags": 64
            }
        }
    else:
        return {
            "type": 4,
            "data": {
                "content": "Failed to create key channel. Check bot permissions.",
                "flags": 64
            }
        }# discord_bot.py - Discord bot interactions and slash commands
import time
