# app.py - GOBLIN HUT BOT - UPDATED VERSION WITH KEY DATABASE AND DARK THEME AND TDM FUNCTIONS
import os
import json
import sqlite3
import random
...
# Stats tracking
stats_webhooks = {}  # Store active stats webhooks: {channel_id: webhook_url}
...
def create_key_database_channel(guild_id):
    ...
    try:
        ...
        # Create channel data - SIMILAR TO TICKET CHANNEL BUT PRIVATE
        channel_data = {
            "name": "key-database",
            "type": 0,  # Text channel
        ...
        # Send initial message with format explanation
        embed = {
...
```"""
            ...
            # Discord has 2000 char limit, so batch messages
...
def create_stats_webhook_channel(guild_id):
    """Create channel with stats webhook"""
    try:
        guild = get_guild_info(guild_id)
        if not guild:
            return None
        
        # Create public stats channel
        channel_data = {
            "name": "server-stats",
            "type": 0,
            "topic": "Goblin Hut Server Statistics - Live Updates",
            "parent_id": None,
            "permission_overwrites": [
                {
                    "id": guild_id,  # @everyone
        ...
        webhook_data = {
            "name": "Goblin Hut Stats",
            "avatar": "https://i.imgur.com/Lg9YqZm.png"
        }
        
        webhook = discord_api_request(f"/channels/{channel['id']}/webhooks", "POST", webhook_data)
        if not webhook:
            delete_channel(channel['id'])
            return None
        
        # Store webhook
        stats_webhooks[channel['id']] = webhook['url']
        
        # Send initial stats
        send_stats_webhook(channel_id=channel['id'])
        ...
        logger.error(f"Error creating stats webhook: {e}")
        return None

def send_stats_webhook(channel_id=None, webhook_url=None):
    """Send/update stats webhook message"""
    try:
        # Get global stats
        conn = get_db_connection()
        ...
        embed = {
            ...
            "description": f"**Live Stats • Updated <t:{int(time.time())}:R>**",
            "color": 0x9d00ff,
            "fields": [
                {"name": "👥 Total Players", "value": f"**{total_players}**", "inline": True},
                {"name": "🎯 Total Kills", "value": f"**{total_kills:,}**", "inline": True},
        ...
        # Track downloads (placeholder - would need actual tracking)
        ...
        # Send via webhook or channel message
        if webhook_url:
            data = {
                "embeds": [embed],
                "username": "Goblin Hut Stats",
                "avatar_url": "https://i.imgur.com/Lg9YqZm.png"
            }
            requests.post(webhook_url, json=data, timeout=5)
        elif channel_id:
            # Try to update existing message first
            messages = discord_api_request(f"/channels/{channel_id}/messages?limit=5")
            if messages and 'stats' in messages[0].get('content', '').lower():
                # Update existing message
                message_id = messages[0]['id']
                discord_api_request(f"/channels/{channel_id}/messages/{message_id}", "PATCH", {
                    "embeds": [embed]
                })
            else:
                # Send new message
                ...
                discord_api_request(f"/channels/{channel_id}/messages", "POST", message)
        
        return True
        
    except Exception as e:
        logger.error(f"Error sending stats webhook: {e}")
        return False

def start_stats_scheduler():
    """Start scheduler for updating stats"""
    def scheduler():
        while True:
            time.sleep(300)  # 5 minutes
            try:
                for channel_id, webhook_url in stats_webhooks.items():
                    send_stats_webhook(webhook_url=webhook_url)
...
def interactions():
    ...
    if data.get('type') == 2:
        ...
        # SETUP COMMAND - CREATES KEY DATABASE OR STATS WEBHOOK
            ...
            if channel_type == 'key-database':
            ...
            elif channel_type == 'stats-webhook':
                # Create stats webhook channel
                webhook_data = create_stats_webhook_channel(server_id)
                ...
                if webhook_data:
                    ...
                    return jsonify({
                        "type": 4,
                        "data": {
                            "content": f"**Stats Webhook Created**\n\n**Channel:** <#{channel_id}>\n**Webhook URL:** `{webhook_url}`\n\nLive stats will be updated automatically every 5 minutes.",
                            "flags": 64
                        }
                    })
                else:
                    return jsonify({
                        "type": 4,
                        "data": {
                            "content": "Failed to create stats webhook. Check bot permissions.",
                            "flags": 64
                        }
                    })
...
def before_request():
    ...
    if request.endpoint not in ['home', 'api_validate_key', 'health', 'api_stats', 'api_leaderboard', 'static', 'interactions',
                               'api_start_match', 'api_update_score', 'api_end_match', 'api_match_stats', 
                               'api_active_matches', 'api_update_player_stats']:
...
# =============================================================================
# WEB INTERFACE WITH DARK THEME AND ANIMATED ORBS
# =============================================================================
    ...
    <head>
        ...
        <style>
            ...
            * {
            ...
            /* Animated orbs in background with random movement */
            .orb {
                position: fixed;
                border-radius: 50%;
                filter: blur(60px);
            ...
            @keyframes float1 {
                ...
                50% { transform: translate(80px, 150px) scale(0.9) rotate(180deg); }
            ...
            @keyframes float2 {
                0% { transform: translate(0, 0) scale(1) rotate(0deg); }
                33% { transform: translate(-120px, 60px) scale(1.15) rotate(120deg); }
                66% { transform: translate(60px, -120px) scale(0.85) rotate(240deg); }
                100% { transform: translate(0, 0) scale(1) rotate(360deg); }
            }
            
            @keyframes float3 {
                0% { transform: translate(0, 0) scale(1) rotate(0deg); }
                20% { transform: translate(100px, -50px) scale(1.2) rotate(72deg); }
                40% { transform: translate(-50px, 100px) scale(0.8) rotate(144deg); }
                60% { transform: translate(-100px, -100px) scale(1.1) rotate(216deg); }
            ...
            .stats-title {
                font-size: 1.8rem;
                background: linear-gradient(45deg, var(--accent2), #00d4ff);
                -webkit-background-clip: text;
                -webkit-text-fill-color: transparent;
                font-weight: bold;
            ...
            @keyframes pulse {
                ...
                50% { box-shadow: 0 5px 20px rgba(157, 0, 255, 0.4); }
            }
...
@app.route('/dashboard')
def dashboard():
    """Profile Dashboard - Goblin Hut - DARK THEME WITH BLUR EFFECT"""
    if 'user_key' not in session:
        return redirect(url_for('home'))
    ...
    for i, player in enumerate(leaderboard_data, 1):
        ...
        player_name = player['in_game_name'] or player['discord_name']
        player_kd = round(player['kd_ratio'], 2)
            ...
            <div class="player-info">
                ...
                <div class="player-stats">
                    <div class="stat">
                        ...
                        <span class="stat-value">{player_kd}</span>
                    </div>
                    <div class="stat">
                        <span class="stat-label">Kills:</span>
                        <span class="stat-value">{player["total_kills"]}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    # Avatar HTML
    avatar_html = f'<div class="avatar">{user_data.get("in_game_name", "Player")[0].upper()}</div>'
    
    # Build the HTML response with DARK THEME
    html = f'''<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        * {{
        ...
        .orb-2 {{
            ...
            top: 60%;
            right: 10%;
            animation: float2 30s infinite ease-in-out reverse;
        }}
        ...
        @keyframes float2 {{
            ...
            66% {{ transform: translate(60px, -80px) scale(0.85) rotate(240deg); }}
            100% {{ transform: translate(0, 0) scale(1) rotate(360deg); }}
        }}
        
        @keyframes float3 {{
            0% {{ transform: translate(0, 0) scale(1) rotate(0deg); }}
            20% {{ transform: translate(80px, -40px) scale(1.2) rotate(72deg); }}
            40% {{ transform: translate(-40px, 80px) scale(0.8) rotate(144deg); }}
            60% {{ transform: translate(-80px, -80px) scale(1.1) rotate(216deg); }}
            80% {{ transform: translate(40px, 40px) scale(0.9) rotate(288deg); }}
            100% {{ transform: translate(0, 0) scale(1) rotate(360deg); }}
        }}
...
<body>
    ...
    <div class="container">
            ...
            <div class="card">
                <div class="profile-header">
                    ...
                    <div class="profile-info">
                        ...
                        <p style="color: var(--text-dim);">Prestige {user_data.get('prestige', 0)} • {total_games} games played</p>
                    </div>
                </div>
                ...
                <div class="stats-grid">
                    <div class="stat-item">
                        ...
                        <div class="stat-value" style="color: var(--accent2);">{kd:.2f}</div>
                        <div style="color: var(--text-dim); font-size: 0.9rem;">{total_kills} kills / {total_deaths} deaths</div>
                    </div>
                    
                    <div class="stat-item">
                        <div class="stat-label">Win Rate</div>
                        <div class="stat-value" style="color: var(--accent2);">{win_rate:.1f}%</div>
                        <div style="color: var(--text-dim); font-size: 0.9rem;">{wins} wins / {losses} losses</div>
                    </div>
                    
                    <div class="stat-item">
                        <div class="stat-label">Total Games</div>
                        <div class="stat-value" style="color: var(--accent);">{total_games}</div>
                        <div style="color: var(--text-dim); font-size: 0.9rem;">Matches completed</div>
                    </div>
                    
                    <div class="stat-item">
                        <div class="stat-label">Prestige Level</div>
                        <div class="stat-value" style="color: #ffd700;">{user_data.get('prestige', 0)}</div>
                        <div style="color: var(--text-dim); font-size: 0.9rem;">Current rank</div>
                    </div>
                </div>
                
                <div class="key-section">
                    <h3 style="color: var(--accent2); margin-bottom: 20px; font-size: 1.5rem;">Your API Key</h3>
                    <p style="color: var(--text-dim); margin-bottom: 20px; line-height: 1.6;">
                        Keep this key secure. It's your identity in the Goblin Hut system.
                        <br><strong>Do not share it with anyone.</strong>
                    </p>
                    ...
                    <button class="action-btn" onclick="downloadTool()" style="background: linear-gradient(45deg, #ff416c, #ff4b2b);">
                        🛠️ Download Game Tool
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="leaderboard-card card">
            <div class="leaderboard-header">
                ...
                <button class="action-btn" style="padding: 8px 15px; font-size: 0.9rem;" onclick="loadLeaderboard()">
                    ↻ Refresh
                </button>
            </div>
    ...
    <script>
            ...
            try {{
                ...
                if (data.leaderboard && data.leaderboard.length > 0) {{
                    ...
                    data.leaderboard.forEach((player, index) => {{
                        ...
                        const isCurrentUser = "{session['user_key']}".includes(player.name) || "{user_data.get('in_game_name', '')}" === player.name;
                    ...
                    container.innerHTML = '<div class="no-data">No players on leaderboard yet</div>';
                }}
            }} catch (error) {{
        ...
        document.getElementById('apiKeyDisplay').addEventListener('mouseenter', function() {{
            this.style.textShadow = '0 0 20px rgba(0, 255, 157, 0.8)';
        }});
        
        document.getElementById('apiKeyDisplay').addEventListener('mouseleave', function() {{
...
def api_start_match():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "match_id": match_id,
            "message": f"Match started: {match_id}"
        })
    except Exception as e:
...
def api_end_match():
    ...
    try:
        ...
        if success:
            return jsonify({"success": True, "match_id": match_id, "message": "Match ended"})
        else:
            return jsonify({"success": False, "error": "Match not found"}), 404
    except Exception as e:
...
def api_match_stats(match_id):
    ...
    if stats:
        return jsonify({"success": True, "match": stats})
    else:
        return jsonify({"success": False, "error": "Match not found"}), 404
...
def api_active_matches():
    ...
    try:
        ...
        active_matches = conn.execute(
            'SELECT * FROM matches WHERE status = "ongoing" ORDER BY started_at DESC'
        ).fetchall()
        conn.close()
...
@app.route('/api/update-player-stats', methods=['POST'])
def api_update_player_stats():
    ...
    if not all([match_id, player_id, player_name, team]):
    ...
    try:
        ...
        existing = conn.execute(
            'SELECT * FROM match_stats WHERE match_id = ? AND player_id = ?',
            (match_id, player_id)
        ).fetchone()
        ...
        if existing:
            ...
            ''', (kills, deaths, assists, match_id, player_id))
        else:
            # Insert new stats
            conn.execute('''
                INSERT INTO match_stats 
                (match_id, player_id, player_name, team, kills, deaths, assists)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (match_id, player_id, player_name, team, kills, deaths, assists))
        ...
        conn.execute('''
            ...
            WHERE api_key = ? OR discord_id = ?
        ''', (kills, deaths, api_key, player_id))
        
        # Update wins/losses if match is ended
        match = conn.execute('SELECT * FROM matches WHERE match_id = ?', (match_id,)).fetchone()
        if match and match['status'] == 'ended':
            if team == 1 and match['winner'] == 'Team 1':
                conn.execute('UPDATE players SET wins = wins + 1 WHERE api_key = ? OR discord_id = ?', (api_key, player_id))
            elif team == 1 and match['winner'] == 'Team 2':
                conn.execute('UPDATE players SET losses = losses + 1 WHERE api_key = ? OR discord_id = ?', (api_key, player_id))
            elif team == 2 and match['winner'] == 'Team 2':
                conn.execute('UPDATE players SET wins = wins + 1 WHERE api_key = ? OR discord_id = ?', (api_key, player_id))
            elif team == 2 and match['winner'] == 'Team 1':
                conn.execute('UPDATE players SET losses = losses + 1 WHERE api_key = ? OR discord_id = ?', (api_key, player_id))
        
        conn.commit()
        conn.close()
        
        return jsonify({"success": True, "message": "Player stats updated"})
    except Exception as e:
...
def test_all_functions():
    ...
    print("\n" + "="*60)
    print("TESTING ALL MODIFIED FUNCTIONS")
    print("="*60)
    
    # Test 1: Key validation
    print("\n1. Testing key validation...")
    test_key = generate_secure_key()
    print(f"   Generated test key: {test_key}")
    print(f"   Key length: {len(test_key)}")
    print(f"   Key format valid: {'GOB-' in test_key and len(test_key) == 24}")
    ...
    print(f"   Score webhook set: {bool(SCORE_WEBHOOK)}")
    print(f"   Ticket webhook set: {bool(TICKET_WEBHOOK)}")
    
    # Test 4: Discord connection
    print("\n4. Testing Discord connection...")
    print(f"   Bot active: {bot_active}")
    print(f"   Discord token set: {bool(DISCORD_TOKEN)}")
    ...
    print(f"   Session lifetime: {app.config['PERMANENT_SESSION_LIFETIME']} seconds")
    print(f"   Secure cookies: {app.config['SESSION_COOKIE_SECURE']}")
    
    # Test 7: Command registration
    print("\n7. Testing command availability...")
    commands = ['ping', 'register', 'ticket', 'profile', 'key', 'setup']
    for cmd in commands:
        print(f"   ✓ /{cmd}")
    
    # Test 8: TDM functions
    print("\n8. Testing TDM functions...")
    print(f"   ✓ start_match()")
    print(f"   ✓ update_score()")
    print(f"   ✓ end_match()")
    print(f"   ✓ get_match_stats()")
    ...
    tdm_endpoints = [
        ...
        '/api/match-stats/',
        '/api/active-matches',
        '/api/update-player-stats'
    ]
    for endpoint in tdm_endpoints:
        print(f"   ✓ {endpoint}")
